...
:18081 页面左侧有「代码」可折叠侧边栏，用于把图上的符号落回源码：

- **目录查看**：左侧面板选项目后自动加载目录树（懒加载、可过滤文件名；.git / node_modules / build 等重型目录默认隐藏），点击目录展开、点击文件打开。
- **代码文件查看**：文件内容带行号 + 轻量语法高亮（关键字/字符串/注释/数字/类型等），大文件截断保护。
- **定位 Neo4j 节点**：在代码里点击某个符号（光标进入符号中间），工具条会显示「行号 · 列号 · 标识符」；点「定位 Neo4j 节点」，按 **文件 + 行号 + 标识符** 反查图数据库，返回**运行时节点**（CalledMethod / Value / Condition）与**非运行时节点**（Class / Method / Field / Value 声明）的命中列表，并把命中节点连同邻居合并进当前 3D 图、高亮对应代码行。点命中行可选中对应 3D 节点。
- 「清除高亮」取消 3D 高亮与代码行标记。

另外，当 `query_graph` / 探索查询返回的是**非图结构（纯标量）结果**（如 `RETURN count(n)`、`RETURN n.name`），3D 图只显示一个「查询结果」节点，但右上控制面板会列出**完整的结果行**（键值对），方便人看真实数值。
