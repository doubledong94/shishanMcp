...
/**
 * Graph 功能的外部依赖配置（scip 网关 / Neo4j / tree-sitter 缓存目录）。
 *
...
export class GraphConfig {
  ...
  /** tree-sitter grammar 下载缓存目录（持久盘，重建容器不用重下）。 */
  readonly tsCacheDir: string;
  ...
  constructor(private readonly data: DataStoreService) {
    ...
    this.tsCacheDir = path.join(this.data.getRoot(), "ts-cache");
