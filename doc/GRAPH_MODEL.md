...
### 4.4 when(表达式):类 if-else-if,守卫先读、恒 2 分叉、分支尾汇入赋值/后续

`when { g1 -> A; g2 -> B; else -> C }` 等价于 if-else-if(每个 WHEN_ENTRY 的守卫 = if 条件,else 为兜底),常用于 `x = when {...}` 表达式。**设计目标**(与 if/loop 一致):

| 位置 | NEXT |
| --- | --- |
| 守卫求值 | `g1 读 → g2 读 → …`(各守卫值先入链)→ when 条件节点 |
| when 条件分叉 | **恒 2**:真→本分支体首、假→下一 when 分支(未匹配才落到下一个守卫/else) |
| 分支体尾 | 汇入**合并点** = 整个 when 之后(若非赋值表达式即 `next`;**若是 `x = when{…}` 则为 `x` 的写**,且先读后写) |
| 合并点入边 | 每条落到底的分支尾(else 分支也算一条) |

要点:
- **when 条件节点应恒 2 分叉**(真→体、假→下一个分支),而**不是**把每个 WHEN_ENTRY 都当一条独立分支(N 叉)——否则守卫读被当成分支目标,结构失真(当前实现有此问题)。
- 守卫**读**在 when 条件**之前**入链(读 → 条件 → 分支)。
- `x = when{…}`:when 分支体尾汇入 `x` 的**写**,顺序为读→写。

> 实现现状(已按 if-else-if 重构):每个非 else 的 WHEN_ENTRY 守卫物化为一个 kind=IF 条件节点(锚在守卫位置,如 okhttp CallServerInterceptor.kt`response = when{…}` 的 when 条件落在守卫 `isUpgradeRequest` 处 #140:10),守卫读先入链、条件恒 2 分叉(真→本分支体首、假→else 体首),所有落到底的分支尾汇入 when 之后的合并点。**`x = when{…}` 的合并点 = `x` 的写**(先读后写):重新赋值(`var x; x = when{…}`)的写也已纳入链——把写延迟到赋值语句末行(when RHS 末行)再入链,分支尾在写处汇合;example 中两条 when 分支尾(149:15、170:16 的 `.build()` 返回)都汇入 `response` 写 #137:6,再续到之后的语句。
...
>
> NEXT 含**循环回边**。Neo4j 变长关系默认 trail 语义（同一条边一次路径内不重复），所以
> **不会死循环、一定有限**；但"有限"不等于"能算完"——**节点数不多，路径数却能爆炸**。
>
> 实测 `CallServerInterceptor.intercept` 一个方法体（NEXT 全程没跨出函数体，终点 379 个节点
> 全在同一文件），三种返回形态：
>
> | 返回形态 | 耗时 | 行数 | 输出大小 |
> | --- | --- | --- | --- |
...
> ```
>
> `count(p)` / `count(length(p))` 最糟（要逐条实体化）；`count(DISTINCT x)` 只要收敛后的
> 节点集，秒回但画不出边。**注意单方法锚定并不足以让 `RETURN p` 安全**——同一个
> `intercept` 一个方法体就有 19238 条路径。
>
