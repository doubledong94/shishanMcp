...
```
数据 ──CONTROLS──► 逻辑 ──NEXT──► 时机的分形(CALLS 调用) ──跨函数──► 时机(NEXT 主轴，在被调方法内)
...
数据 D=`FLOWS`（写→读）、逻辑 L=`CONTROLS→NEXT`（守卫值→分支经NEXT链到调用点）、调用分形 C=`CALLS`（调用点→被调方法，时序轴的递归展开）、时序主轴 A=`NEXT*`（最早→最晚）、成员/下标分形 R=`REF/INDEX`（实例→成员）。
