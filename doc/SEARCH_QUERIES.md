...
## 1. 从高层函数沿「时机」向下展开（原始搜索句）

定位一个高层函数后，把它的整个调用链（含分支/占用）在时机维度展开——**不显示 Value（数据）节点**。
...
## 2. 函数体内「一层函数按顺序相连」（顺序边搜索句）

不新建边，只沿库里已有的 `NEXT` 顺序边，把 `intercept` 函数体（一层内）的相邻调用按执行顺序连起来。
...
- `ALL(n IN nodes(p)[1..-1] WHERE NOT n:CalledMethod)`：抽象到"函数层"，`a→b` 之间只穿 Value/Condition，
  即 `a` 在顺序链上的**下一个函数**就是 `b`。
- 一个 `a` 有多个后继 = **分支**（if/else/循环各连各的），是正当的控制流，不是漏连。
- 实测返回从 `currentTimeMillis@36` 一路到 `responseHeadersEnd@126` 的顺序链。

> 局限：`NEXT` 边在绘制上是被中间 Value 拆开的（见 §5），这条只用来**列出/理解**顺序，不是直接渲染的边集。

---

## 3. 函数体「完整顺序链」（把 NEXT 渲染进图，含 Value 连接件）
